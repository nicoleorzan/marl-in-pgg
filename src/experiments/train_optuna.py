...
from src.experiments.utils_train_reinforce import eval, find_max_min
...
def objective(trial, args, repo_name):
    ...
    wandb.init(project=repo_name, entity=args.wandb_entity, config=all_params, mode=args.wandb_mode)#, sync_tensorboard=True)
    config = wandb.config
    print("config=", config)
    ...
    for epoch in range(config.n_epochs): 
        ...
        for m in config.mult_fact:
            ...
            mex_distrib_given_m[m] = mex_distrib
